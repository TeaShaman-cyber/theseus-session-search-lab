...
class ChatGPTExportAdapterTest(unittest.TestCase):
    ...
    def test_single_leaf_chatgpt_snapshot_reconciles_after_branched_family_appears(self):
        from session_search.chatgpt_export import ingest_export, materialize_export
        from session_search.corpus_store import (
            CorpusPaths,
            ingest_artifact,
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            single_source = self.write_export(
                ...
                [
                    self.conversation(
                        cid="route-single-then-branched",
                        current="b",
            ...
            full_source = self.write_export(
                root,
                [self.conversation(cid="route-single-then-branched", current="b")],
            )
            ...
            branch_id = next(
                ...
                if session_id.startswith("route-single-then-branched~branch-")
            )

            corpus = root / "corpus"
            self.assertEqual(ingest_artifact(single_child, corpus)["status"], "INGESTED")
            ...
            with sqlite3.connect(CorpusPaths.from_root(corpus).db) as conn:
                ...
                self.assertEqual(route, (branch_id, "BRANCH_MATCH"))
                self.assertEqual(
                    {
                        row[0]
